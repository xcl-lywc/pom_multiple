...
<template>
  <div v-if="homeDetail">
  	...
  	<el-container class="home-container" v-if="loginInUserInfo"> 
        ...
        <el-dropdown class=" fr" split-button type="primary" @command="dropdownHandleCommand">
          组织列表
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item  
              v-for="(item, index) in userOrg.rows" :key="index"
              :command="item.org_id">
              {{item.org_name}}
            </el-dropdown-item> 
          </el-dropdown-menu>
  ...
  import ThemePicker from '../../../../components/ThemePicker.vue' 
    ...
    watch: {
      ...
      getQueryBlockData(){  
        this.axios.get( `${this.common.basePath}/home/user/home`, {} ).then( (response) => {   
          ...
          if(this.common.getParamsByName('orgId')){
            sessionStorage.setItem('orgId', this.common.getParamsByName('orgId')); 
          }else{
      ...
      */
       ...
       */
      getQueryUserOrgList(){
        this.axios.post( `${this.common.basePath}/user/orgs/getAll`, {} ).then( (response) => {  
